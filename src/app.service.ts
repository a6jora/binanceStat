...
export class AppService {
    async get(): Promise<string> {
        let address = "0x0E09FaBB73Bd3Ade0a17ECC321fD13a19e81cE82";
        ...
        for (let i = 0; i < 1000; i++) {
            let data = await contract.getPastEvents("Transfer",
                {
                    fromBlock: 693963 + i * 1000,
                    toBlock: 693963 + (i + 1) * 1000, // You can also specify 'latest'
                });
            data.forEach(function (value) {
                if (!breakPoint) {
                    // значение, полученое эмпирическим путем
